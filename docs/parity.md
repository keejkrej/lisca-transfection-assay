...
Both CLIs should write the same `results/<sample>/` PNG basenames for per-sample
packs (one axes per sample; no `*_shared_y`):

- `baseline_intensity.png`, `protein_lifetime.png`, `mrna_lifetime.png`,
  `onset_time.png`, `expression_rate.png`, `expression_rate_log.png`
- `traces_fit.png`

Plus one multi-sample scatter at `results/expression_rate_vs_onset_time.png`
(one subplot per sample, same `subplot_grid_shape` / `figure_size_for_panels`
as traces; Pearson r and n on each panel; successful finite fits only).
...
write `results/expression_rate_vs_onset_time.png` (file exists and is
